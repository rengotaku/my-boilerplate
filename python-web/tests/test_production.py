...
class TestMakefileStartTarget:
    """make start ターゲットの本番サーバー要件テスト."""

    def test_start_target_exists_in_makefile(self) -> None:
        """Makefile に start ターゲットが定義されていること."""
        content = _read_makefile()
        assert "start:" in content or "start: build" in content

    def test_start_target_does_not_enable_reload(self) -> None:
        """start ターゲットが --reload を有効化しないこと.

        本番サーバーでは自動リロードを無効にする。
        uvicorn 0.44 では --no-reload フラグは廃止 (デフォルトが非リロード)。
        """
        content = _read_makefile()
        start_section = _extract_target_section(content, "start")
        assert not re.search(r"--reload(\s|$)", start_section), (
            "start target で --reload が有効化されている。"
            f"section: {start_section}"
        )

    def test_start_target_depends_on_build(self) -> None:
        """start ターゲットが build に依存していること."""
        content = _read_makefile()
        lines = content.split("\n")
        for line in lines:
            if line.startswith("start:"):
                ...
                return
        pytest.fail("start target not found")

    def test_start_target_uses_uvicorn(self) -> None:
        """start ターゲットが uvicorn を使用すること."""
        content = _read_makefile()
        start_section = _extract_target_section(content, "start")
        assert "uvicorn" in start_section

    def test_start_target_uses_configurable_port(self) -> None:
        """start ターゲットが $(PORT) で設定可能なこと."""
        content = _read_makefile()
        assert "PORT" in content
        start_section = _extract_target_section(content, "start")
        assert "$(PORT)" in start_section

    def test_start_target_binds_to_all_interfaces(self) -> None:
        """start ターゲットが 0.0.0.0 にバインドすること."""
        content = _read_makefile()
        start_section = _extract_target_section(content, "start")
        assert "0.0.0.0" in start_section


class TestRunVsStartDifferences:
    """開発モード (run) と本番モード (start) の違いが正しいこと."""

    def test_run_uses_reload_start_does_not(self) -> None:
        """run は --reload あり、start は --reload を含まない.

        uvicorn 0.44 で --no-reload は廃止されたので、start では --reload を
