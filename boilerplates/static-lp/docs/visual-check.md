...
レイアウト崩れ・テキストのはみ出し・CTA の視認性は、`rules/testing.md`
「静的コンテンツにテストを作り込まない」方針のとおり、原稿と描画結果を突き合わせる
同語反復テストでは検知しにくい（このテンプレートの `CLAUDE.md` 参照）。代わりに
...
`http://localhost:5173/` の部分は、手順1で実際に上がったポートに合わせて置き換える。
...
本テンプレートの初期構成（Header + 本文）はセクション数が少ないため上記で十分だが、
