...
`go-cli` をベースに、外部 CLI（codex / agy などの AI CLI に限らず、任意の外部コマンド）をラップして実行する Go CLI アプリケーションのテンプレート。codex-run / agy-run で個別に実装されていた「外部コマンド実行 + timeout + プロセスグループ kill」「rate-limit/quota シグネチャ検出」「候補を順に試すフォールバック」「fake コマンドでのテスト」を、汎用パッケージ群として抽出・同梱している。
...
`context` の timeout、プロセスグループ単位の kill（`Setpgid: true` + 負の PID への `SIGKILL`）、stdout/stderr を分離したキャプチャを備えた `Run(ctx, Options) (Result, error)`。

- 抽出元: agy-run `internal/runner` の attempt 実行 + `killGroup`、codex-run `internal/runner` の `runAttempt`
- 設計意図: 外部 CLI がハングしたときに `context` の cancel だけでは子プロセスが生き残ることがある（外部 CLI がさらに孫プロセスを起動するケース）。`Setpgid` でプロセスグループを分離し、cancel 時にグループごと `SIGKILL` することで、ハングした外部 CLI とその子孫プロセスを確実に回収する
...
- 設計意図: AI CLI（や外部 API をラップする CLI 全般）の rate-limit エラーは exit code だけでは判別できず、stdout/stderr のテキストにしか手がかりが無いことが多い。正規表現ベースのシグネチャ検出を独立パッケージに切り出し、`fallback` の retryable 判定と個別に単体テストできるようにした
...
- 設計意図: 「rate-limit で落ちたら次のモデル/エンドポイントを試す」という制御フローは両リポジトリで同型に実装されていたが、retryable 判定（`logscan`）と実行（`execx`）を直接埋め込んでいたためテストが難しかった。3 者を分離することで、`fallback` のテストは `execx.Run` の代わりに `RunnerFunc` の fake を渡すだけで済むようになっている

## やらないこと（意図的にスコープ外）

- codex / agy / gemini 固有のプロトコル（stdout JSON 契約・conversation-id・slot/flock による並列制御・watchdog）は持ち込まない。exec/timeout/fallback/logscan/envx/fakebin の 6 点に絞った汎用パッケージのみを提供する（詳細は `docs/adr/0001-extract-templates-from-recent-repos.md` 参照）
