...
SERVERS := go-rest-api go-graphql-api go-grpc-api go-ssr-web go-ssr-web-minimal go-react-spa go-react-admin react-spa

## stop-all: Stop all dev servers
stop-all:
	@for svc in $(SERVERS); do $(MAKE) -s -C boilerplates/$$svc stop 2>/dev/null || true; done
	@echo "All servers stopped"

## status: Check all dev servers
status:
	@for svc in $(SERVERS); do $(MAKE) -s -C boilerplates/$$svc status 2>/dev/null || true; done
